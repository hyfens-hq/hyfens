...
The v0.1.7 integration is a local stdio process. It is not a hosted MCP
...
The v0.1.7 MCP surface is intentionally small:

- local stdio launched by `hyfens mcp`;
- server identity `hyfens`, version `0.1.7`;
...
1. Install a v0.1.7 Hyfens CLI and make `hyfens` available on `PATH`. A source
...
The command below starts the v0.1.7 MCP stdio server and waits for protocol
messages:
...
The v0.1.7 core catalog has these exact public names. Tool descriptions mark
...
Install the v0.1.7 CLI or put the CLI wrapper from the [CLI reference](cli.md)
...
resolved path and version, then install or build the v0.1.7 CLI that includes
...
using a v0.1.7 binary and that no other process is consuming the same stdio
stream.
...
later. They are not part of the v0.1.7 contract described here. A future
