...
launch prints a short running message on stderr, then may appear idle until a
client sends an MCP request; that is expected. The running message never goes
to stdout because stdout is reserved for MCP protocol traffic.
...
results rather than terminal output. MCP tool names are scoped by the process
configuration; the binary currently exposes the names below without an
additional `hyfens_` prefix.

- `status` — Read-only. Reports the local toolchain, selected project, and
  bounded artifact inventory.
- `doctor` — Read-only. Checks supported Flutter/Dart and project
  prerequisites.
- `profile_list` — Read-only. Lists profile metadata without credential
  material.
- `profile_current` — Read-only. Reports the selected profile and non-secret
  endpoint/scope metadata.
- `profile_get` — Read-only. Reads one named profile's metadata without
  credential material.
- `project_init` — Mutation. Initializes or updates the bounded local Hyfens
  project binding.
- `release_create` — Mutation. Creates a supported release baseline/artifact
  locally.
- `release_inspect` — Read-only. Reads bounded release metadata.
- `patch_create` — Mutation. Analyzes and signs a supported patch artifact
  locally.
- `patch_verify` — Read-only. Verifies patch metadata, signature, and exact
  release binding.
- `patch_inspect` — Read-only. Reads bounded patch metadata without returning
  bytecode or signature bytes.
- `deploy` — Mutation-shaped capability probe. The current public toolchain
  reports an honest unsupported result because its authenticated delivery
  sequence is not yet a shared public service.
- `rollback` — Mutation. Records a bounded local rollback to the trusted base
  while preserving the patch sequence high-water mark.
- `control_plane_discovery` — Read-only. Reads non-secret compatibility
  capabilities for the selected profile.

The available arguments, supported artifact kinds, project-path rules, and
result fields are those returned by `tools/list` for the running binary.
Internal CLI commands and APIs are not automatically MCP tools.
...
Mutation tools are explicit operations. `project_init`, `release_create`, and
`patch_create` affect local project or artifact state; `deploy` and `rollback`
can affect remote control-plane state.
