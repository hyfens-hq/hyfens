...
The v0.1.8 integration is a local stdio process. It is not a hosted MCP
...
## Current v0.1.8 boundary

The v0.1.8 MCP surface is intentionally small:

- local stdio launched by `hyfens mcp`;
- server identity `hyfens`, version `0.1.8`;
...
1. Install a v0.1.8 Hyfens CLI and make `hyfens` available on `PATH`. A source
...
The command below starts the v0.1.8 MCP stdio server and waits for protocol
messages:
...
The v0.1.8 core catalog has these exact public names. Tool descriptions mark
...
- `hyfens_doctor` — Read-only. Checks supported Flutter/Dart and project
  prerequisites.
- `hyfens_analyze` — Read-only. Reports the shared typed compatibility result
  for a selected release without returning application source or patch bytes.
...
Install the v0.1.8 CLI or put the CLI wrapper from the [CLI reference](cli.md)
...
resolved path and version, then install or build the v0.1.8 CLI that includes
...
using a v0.1.8 binary and that no other process is consuming the same stdio
stream.
...
later. They are not part of the v0.1.8 contract described here. A future
