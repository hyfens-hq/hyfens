...
This guide runs the bounded Hyfens developer workflow against a self-hosted
control plane. The CLI uses repository path
...
Provide the HTTPS API base or host once and name the profile:
...
[product API domain model](spec/product-api-domain.md) for the public surface
and invariants.
